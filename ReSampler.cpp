	...
	try {
		if (bUseDoublePrecision) {
			...
			if (ci.dsfInput) {
				...
				return parallelConvert<SndfileHandle, double>(ci) ? EXIT_SUCCESS : EXIT_FAILURE;
			}
		}
		else {
			if (ci.dsfInput) {
				...
				return parallelConvert<SndfileHandle, float>(ci) ? EXIT_SUCCESS : EXIT_FAILURE;
			}
		}
	}
...
template<typename FileReader, typename FloatType>
bool parallelConvert(const conversionInfo& ci, bool peakDetection)
{
	// Open input file:
	FileReader infile(ci.InputFilename);
