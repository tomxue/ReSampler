...
**--ns &lt;n&gt;** select [dither profile](./ditherProfiles.md) from 0-13. Generally speaking, as the dither profile number increases, the noise-shaping curve gets  progressively more "intense" (higher amplitude). Dither profile 0 is completely flat (no noise shaping), and is equivalent to **--flat-tpdf**. The default dither profile (if no profile is specified) is #6 (standard), which has a moderate noise-shaping curve.
...
**--rf64** forces output .wav file to be in rf64 format. Has no effect if output file is not a .wav file.

*Note: If your output file has an .rf64 extension, it will automatically be in rf64 format*

**--noPeakChunk** suppresse output of PEAK chunk in floating-point formats
