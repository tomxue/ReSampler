...
#define FIR_NOISE_SHAPING_FILTER_SIZE 57
...
#else
	// IIR Noise Shaping:
	FloatType shapedNoise = f2.filter(f1.filter(tpdfNoise)); // filter the triangular noise with two cascaded biQuads 
...
private:
	...
	const double NoiseShapeCoeffs[FIR_NOISE_SHAPING_FILTER_SIZE] = {
		// FIR coefficients:
